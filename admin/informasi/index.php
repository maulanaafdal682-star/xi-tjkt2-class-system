<?php
/**
 * Admin - Manajemen Informasi Kelas
 */
...
try {
    $stmt = $pdo->query('SELECT * FROM class_information ORDER BY created_at DESC');
...
<head>
    ...
    <title>Manajemen Informasi - Admin</title>
...
<body>
    <nav class="navbar">
        <div class="navbar-container">
            <div class="navbar-brand">Admin - Informasi Kelas</div>
    ...
    <div class="container">
        <div class="header">
            <h1 style="color: #1e3a8a; margin: 0;">ℹ️ Informasi Kelas</h1>
            <a href="#tambah" class="btn-primary">+ Tambah</a>
        </div>
        
        <?php foreach ($informasi as $item): ?>
            <div class="card">
                <h3 style="margin: 0 0 10px 0; color: #1e3a8a;"><?= esc($item['judul']) ?></h3>
                <p style="color: #6b7280; font-size: 12px; margin: 5px 0;">📅 <?= formatDate($item['created_at']) ?> | Status: <?= $item['status'] ?></p>
                <div>
